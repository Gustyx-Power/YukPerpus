<x-filament-panels::page>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        ...
        @forelse($books as $book)
            <div class="bg-gray-100 dark:bg-gray-800 rounded-2xl shadow-lg flex flex-col items-center p-0 overflow-hidden" style="min-height: 370px;">
                <div class="w-full flex-1 flex items-center justify-center bg-red-400" style="height: 220px;">
                    <img src="{{ $book->cover ? Storage::url($book->cover) : 'https://via.placeholder.com/180x220?text=No+Cover' }}" alt="Cover" class="object-cover w-full h-full rounded-t-2xl" style="max-height: 220px;">
                </div>
                <div class="w-full px-4 py-3 flex flex-col items-start">
                    ...
                    <div class="flex gap-2 w-full">
                        <button
                            class="flex-1 px-3 py-1 bg-gray-800 text-white rounded-md hover:bg-gray-900 transition text-sm"
                        ...
                        <button
                            class="flex-1 px-3 py-1 bg-primary-600 text-white rounded-md hover:bg-primary-700 transition text-sm"
    ...
    <div id="borrow-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md relative">
            ...
            <form id="borrow-form" method="POST" action="{{ route('book-borrow.store') }}">
                @csrf
